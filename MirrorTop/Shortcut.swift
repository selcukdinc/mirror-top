...
/// Kullan\u0131c\u0131n\u0131n \u00f6zelle\u015ftirebilece\u011fi global k\u0131sayol modeli.
/// Carbon `RegisterEventHotKey` ile uyumlu (`keyCode` Carbon kVK_*, `modifiers` cmdKey/optionKey/shiftKey/controlKey bit maskeleri).
public struct Shortcut: Codable, Equatable, Hashable, Sendable {
    public var keyCode: UInt32
    /// Carbon modifier bit maskesi: `cmdKey | optionKey | shiftKey | controlKey`.
    ...
    /// Varsayılanlar
    public static let defaultCapture     = Shortcut(keyCode: UInt32(kVK_ANSI_T), modifiers: UInt32(cmdKey | optionKey))
    public static let defaultInteraction = Shortcut(keyCode: UInt32(kVK_ANSI_I), modifiers: UInt32(cmdKey | optionKey))
    ...
    public var displayString: String {
        ...
        s += KeyCodeMap.symbol(for: keyCode)
        return s
    }
    
    /// NSEvent'ten Shortcut üretir. Ana modifier (Cmd/Opt/Shift/Ctrl) yoksa nil.
    public static func from(event: NSEvent) -> Shortcut? {
        ...
        // En az bir modifier zorunlu (yoksa global hotkey çakışır).
        guard carbonMods != 0 else { return nil }
        
        return Shortcut(keyCode: UInt32(event.keyCode), modifiers: carbonMods)
    }
}

// MARK: - Key Code → Sembol haritası
